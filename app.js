...
function calcAll() {
  ...
  let totalBeforeDiscounts = subtotal + profit;

  const rabattProzent = n('discountPercent');
  const rabattPauschal = n('discountFixed');
  const skontoProzent = n('cashDiscount');

  const rabattAufschlagProzent =
    rabattProzent > 0
      ? totalBeforeDiscounts / (1 - rabattProzent / 100) - totalBeforeDiscounts
      : 0;

  const discount = rabattAufschlagProzent + rabattPauschal;

  const totalBeforeSkonto = totalBeforeDiscounts + discount;

  const skonto =
    skontoProzent > 0
      ? totalBeforeSkonto / (1 - skontoProzent / 100) - totalBeforeSkonto
      : 0;

  const total = totalBeforeSkonto + skonto;
  const afterSkonto = totalBeforeDiscounts;
